...
class Alarm extends Class {
  async create (data) {
    ...
    if (userId !== relatedUserId) {
      this.conn.query(`INSERT INTO alarm (type, alarm_user_ID, alarm_relatedUser_ID, alarm_board_ID, alarm_article_ID, alarm_comment_ID, alarm_message_ID) VALUES (?, ?, ?, ?, ?, ?, ?)`, [type, userId, relatedUserId, boardId, articleId, commentId, messageId]);
    }
  }
          ...
          case 'newComment':
            ...
            if(notification){
              async()=>{
                ...
                const player = await this.conn.query("SELECT * FROM user WHERE id=?", [user.id]);
          ...
          case 'replyComment':
            ...
            if(notification_reply){
              async()=>{
                ...
                const player_reply = await this.conn.query("SELECT * FROM user WHERE id=?", [user.id]);
